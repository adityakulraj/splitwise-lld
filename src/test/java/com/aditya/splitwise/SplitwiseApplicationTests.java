...
class SplitwiseApplicationTests {
    ...
    public void defaultTest() throws IllegalAccessException {
       ...
       Assertions.assertEquals(balanceMap.get("A").getAmount(), 50.0, 0.0001);
       Assertions.assertEquals(balanceMap.get("B").getAmount(), 30.0, 0.0001);
       Assertions.assertEquals(balanceMap.get("C").getAmount(), -80.0, 0.0001);
    }
    ...
    private HashMap<String, Group> getGroups() {
        ...
        userList.add("B");
        userList.add("C");
        groups.put("123", new Group("Europe", "Euro trip", userList));
        return groups;
    }
    ...
    public void paymentGraphTest() throws IllegalAccessException {
        ...
        Assertions.assertEquals(graph.get("A").getBalances().get("E").getAmount(), 90.0, 0.0001);
        Assertions.assertEquals(graph.get("D").getBalances().get("B").getAmount(), 70.0, 0.0001);
        Assertions.assertEquals(graph.get("F").getBalances().get("C").getAmount(), 40.0, 0.0001);
        Assertions.assertEquals(graph.get("D").getBalances().get("E").getAmount(), 10.0, 0.0001);
